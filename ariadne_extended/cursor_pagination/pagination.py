"""
From django-cursor-pagination==0.1.4
...
class CursorPage(Sequence):
    ...
    def cursor(self, node):
        return self.paginator.cursor(node)


class CursorPaginator:
    def paginate_queryset(self, qs, request, *args, **kwargs):
        ordering = kwargs.pop("ordering", None)
        return InternalPaginator(qs, ordering).page(*args, **kwargs)


class InternalPaginator:
    delimiter = "|"
    invalid_cursor_message = _("Invalid cursor")

    def __init__(self, qs, ordering):
        self.queryset = qs.order_by(*ordering)
    ...
    def page(self, first=None, last=None, after=None, before=None, **kwargs):
        page_size = first or last

        if page_size is None:
            return CursorPage(self.queryset, self)
        ...
        # is limited to +1 of last or first count, used to check for additional items
        total_count = qs.count()
        items = list(qs)[:page_size]
        if last is not None:
            items.reverse()
        has_additional = total_count > len(items)
        additional_kwargs = {}
        if first is not None:
